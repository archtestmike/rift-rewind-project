...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Render recent lookups initially
  renderRecent(recentEl);

  // If user types a tag that implies a shard, nudge the selector and flash
  riotInput.addEventListener('blur', () => {
    ...
    if (auto && regionSel.value !== auto) {
      ...
      regionSel.classList.add('flash'); setTimeout(()=>regionSel.classList.remove('flash'), 800);
      showNote(resultsEl, `Detected region from tag <b>#${escapeHtml(tag)}</b> → set to <b>${auto.toUpperCase()}</b>.`);
    }
  });

  // Allow clicking a recent lookup to fill the input
  recentEl.addEventListener('click', (e) => {
    const a = e.target.closest('a[data-riotid]');
    if (!a) return;
    e.preventDefault();
    riotInput.value = a.getAttribute('data-riotid');
    riotInput.focus();
  });
...
function renderResult(data, ms=0){
  ...
  const champRows = champs.map(c => {
    ...
    const roleClass = tags[0] ? `role-${tags[0]}` : '';

    return `
      <div class="champ ${roleClass}">
        <div>
...
/* ---------- Recent lookups ---------- */
function getRecent(){
  try { return JSON.parse(localStorage.getItem('recentRiotIds')||'[]'); } catch { return []; }
}
function saveRecent(riotId){
  const arr = getRecent();
  arr.unshift(riotId);
  const unique = [...new Set(arr)].slice(0,3);
  try { localStorage.setItem('recentRiotIds', JSON.stringify(unique)); } catch {}
}
function renderRecent(container){
  if (!container) return;
  const items = getRecent();
  if (!items.length){ container.innerHTML = ''; return; }
  container.innerHTML = `Recent: ${items.map(r => `<a href="#" data-riotid="${escapeHtml(r)}">${escapeHtml(r)}</a>`).join(' • ')}`;
}
